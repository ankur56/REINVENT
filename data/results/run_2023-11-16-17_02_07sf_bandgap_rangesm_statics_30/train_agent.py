...
def train_agent(restore_prior_from='data/Prior.ckpt',
    ...
    # Scoring_function
    scoring_function_call = get_scoring_function(scoring_function=scoring_function, num_processes=num_processes, batch_size=batch_size,
    ...
    for step in range(n_steps):
        ...
        batch_score = sum(2 < b < 3 for b in bandgap)
        ...
        if sigma_mode == 'prior_reward':
            ...
            lambda_2 = 1
            sigma = lambda_2*np.abs(prior_likelihood)
        ...
        if sigma_mode == 'prior_greedy':
            ...
            if r < epsilon:
            	...
            	sigma = lambda_2*np.abs(prior_orig)
        ...
        if not save_dir:
            if sigma_mode == 'static': 
                save_dir = 'data/results/run_' + time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime()) + '_sf_' + scoring_function + '_sm_' + sigma_mode + '_s_' + str(sigma)
            elif sigma_mode == 'prior':
                save_dir = 'data/results/run_' + time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime()) + '_sf_' + scoring_function + '_sm_' + sigma_mode + '_l_' + str(lambda_)
            else:
    ...
    prior_likelihood = prior_likelihood.data.cpu().numpy()
    smiles = seq_to_smiles(seqs, voc)
    score = scoring_function_call(smiles)
